  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return asyncDetail.when(
      loading: () => Scaffold(
        ...
        body: const LoadingIndicator(message: '加载分析...'),
      ),
      error: (e, _) => Scaffold(
        appBar: AppBar(title: const Text('菜谱分析')),
      ...
      data: (detail) {
        ...
        final maxCost = detail.ingredients.isEmpty
            ? 0.0
            : detail.ingredients.map((i) => i.estimatedCost ?? 0).reduce((a, b) => a > b ? a : b);
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              Text('\u00A5${cost.toStringAsFixed(2)}', style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold)),
                              Text('${pct.toStringAsFixed(1)}%', style: theme.textTheme.labelSmall?.copyWith(color: theme.colorScheme.outline)),
                            ],
                          ),
                          const SizedBox(height: 4),
                          ClipRRect(
                            borderRadius: BorderRadius.circular(4),
                            child: LinearProgressIndicator(value: barWidth, minHeight: 8, backgroundColor: theme.colorScheme.surfaceContainerHighest),
                          ),
                        ],
                      ),
                    );
