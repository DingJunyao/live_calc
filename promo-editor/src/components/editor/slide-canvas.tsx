...
export function getCanvas(device: Device, orientation: Orientation) {
  ...
  if ((device === "android-7" || device === "android-10" || device === "ipad") && orientation === "landscape") {
...
function getFrameAspect(device: Device, orientation: Orientation) {
  switch (device) {
    ...
    case "ipad":        return orientation === "landscape" ? 8 / 5 : IPAD_RATIO;
    case "android-7":
  ...
  switch (device) {
    ...
    case "ipad":
      if (orientation === "landscape") {
        return { Comp: AndroidTabletL, widthFn: tabletLW, smallWidthFn: (cW, cH) => tabletLW(cW, cH, 0.42) };
      }
