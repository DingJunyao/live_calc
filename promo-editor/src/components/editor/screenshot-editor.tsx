...
export function ScreenshotEditor() {
  ...
  // Tablets default to landscape; phones default to portrait.
  React.useEffect(() => {
    const isTablet = state.device === "ipad" || state.device === "android-7" || state.device === "android-10";
    if (isTablet && supportsLandscape(state.device) && state.orientation !== "landscape") {
      setState((p) => ({ ...p, orientation: "landscape" }));
    } else if (!isTablet && state.orientation !== "portrait") {
