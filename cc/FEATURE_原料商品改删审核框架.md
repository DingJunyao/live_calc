...
### 增强 7：商品营养编辑放开走审核（2026-07-01）
上面「遗留」的商品营养端点 `edit_product_nutrition` 放开走审核，与原料营养对齐。新建 `ProductNutritionExecutor`（entity_type="product_nutrition"，写 `Product.custom_nutrition_data` + `custom_nutrition_source` + snapshot/revert，同构 UsdaProductMatchExecutor 但写用户编辑的 structured_nutrients 不调 matcher）；bootstrap 注册全 manual；端点分流（保留 structured_nutrients 构建 + NRV 计算逻辑，构建后走提议，policy_override 有数据 manual / 无数据补空 auto）；[ProductDetail saveNutritionEdit](../frontend/src/views/products/ProductDetail.vue#L2465) 前端按审核状态提示（读 message：pending→info 待审不刷新 / 管理员+补空→success 刷新）。3 新测试（apply/revert + 有数据 manual + 无数据 auto），19 passed + build 通过。

**续修（PUT 清空路径）**：商品营养「全删除保存」走 PUT `/products/entity/{id}/nutrition`（[update_product_nutrition](../backend/app/api/products_entity.py#L900)，传 null 清空），原也锁管理员（普通用户 403）。同款分流修复：去 admin 锁 + 复用 ProductNutritionExecutor + policy_override（有数据 manual / 无数据补空 auto），前端 PUT 分支按审核状态提示（读 message），2 新测试（普通用户全删除有数据→待审不清空 / 管理员即时清空），21 passed。
