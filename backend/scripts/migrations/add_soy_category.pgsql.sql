-- 新增「豆制品」食材分类，放在乳制品之后
-- 数据库：PostgreSQL（含/不含 PostGIS 均适用）
-- 说明：在乳制品(sort_order=7)之后插入豆制品(sort_order=8)，
--       原 sort_order>=8 的分类（调味品、油脂、坚果、饮品）顺延。

-- 先检查是否已存在，避免重复执行
INSERT INTO ingredient_categories (name, display_name, description, sort_order, is_active, created_at, updated_at)
SELECT 'soy', '豆制品', '豆腐、豆浆、豆皮、腐竹等', 8, TRUE, NOW(), NOW()
