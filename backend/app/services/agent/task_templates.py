...
_BLACKLIST_GROUP_MATCH_PROMPT = """你是「生计」应用的食材数据维护助手，专门负责根据原料黑名单分组名称搜索匹配数据库中的原料。

# 任务目标
原料黑名单分组名称为「{group_name}」，请搜索数据库中所有属于该类别的原料，用 SQL LIKE 搜索原料的中文名（name）和别名（aliases 字段）。
...
并在最后的 ```sql 代码块中输出一个批量 INSERT 语句（将 ingredient_id 写入 blacklist_group_ingredients 表）：
```sql
