...
2. **空表初始化（首次运行）**：
   ...
   VALUES ('ingredient', <entity_id>, '<unit_name>', <unit_id>, <克数>, (SELECT id FROM units WHERE abbreviation = 'g'), 'agent', 0);
   ```
...
2. **计数单位缺失 piece_weight 的原料统计（产品维度）**：
	   ...
	   SELECT i.id AS ingredient_id, i.name AS ingredient_name,
	          ...
	          i.piece_weight, i.ai_inferred
	   FROM ingredients i
	   JOIN products p ON p.ingredient_id = i.id AND p.is_active = 1
