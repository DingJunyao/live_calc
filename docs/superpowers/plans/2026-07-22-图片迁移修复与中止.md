...
- 后端虚拟环境在项目根 `.venv`（即 `d:\code\livecalc\.venv`），测试/编译用它。
- 不要在对话中启动前后端服务（已自动重载）。
...
```
d:\code\livecalc\.venv\Scripts\python.exe -m pytest backend/tests/services/test_storage_migrate.py::test_progress_callback_merges_counts_and_signals_cancel -v
```
...
```
d:\code\livecalc\.venv\Scripts\python.exe -m pytest backend/tests/services/test_storage_migrate.py -k "strip or from_s3" -v
```
...
```
d:\code\livecalc\.venv\Scripts\python.exe -m pytest backend/tests/services/test_storage_migrate.py -k "to_s3" -v
```
...
```
d:\code\livecalc\.venv\Scripts\python.exe -m pytest backend/tests/services/test_storage_migrate.py -k "run_import_task" -v
```
...
```
d:\code\livecalc\.venv\Scripts\python.exe -m py_compile backend\app\services\storage\migrate.py backend\app\services\importer\api_service.py
```
Expected: 无输出（编译通过）。
...
```
d:\code\livecalc\.venv\Scripts\python.exe -m pytest backend/tests/services/test_storage_migrate.py -v
```
