...
某个商品想独立出来单独管理（比如它有自己的营养、自己的价格历史），可以"**拆分为原料**"：把它从当前原料里分出来，成为一个同名原料，原商品的属性/营养随之迁移。同名冲突时支持重命名。

## 原料合并

把多个原料合并成一个（所有引用——菜谱食材、价格、营养——迁移到目标原料）：

- 管理员可合并任意原料
- 普通用户只能合并**自己创建**的源原料到**自己创建**的目标原料
