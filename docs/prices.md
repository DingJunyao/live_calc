...
- **日期**：默认今天（按你本地时区）
- **记录类型**：
  - **价格记录**（默认）：用于成本/趋势计算，不计入你的支出报告
  - **支出**：作为实际消费计入支出报告

记完即生效。价格记录是你的**私有数据**，只有你和管理员能看到原始记录。
...
1. **选商家**：进页面先选一家商家
2. **自动列历史商品**：系统按你过去记过的商品列出来（默认按拼音/字母排序）
3. **逐行填价**：每行一个商品，填价格即可（数量/单位默认 1/斤，可点改）
4. **按填写顺序排序**：可在商家设置里开启"按填写顺序排序"，列表就按你这次填的顺序排，而不是拼音
5. **新增行**：可搜索商品加行
6. **保存**：只保存填了价格的商品
...
快速填写页还有"**粘贴导入**"——把一堆价直接粘进来：

支持的格式（每行一条）：

- `名称 价格` → 如 `鸡蛋 15`
- `名称 价格/单位` → 如 `鸡蛋 15/斤`
- `名称 价格/数量/单位` → 如 `鸡蛋 15/10/个`
- `名称 数量单位 价格` → 如 `鸡蛋 10个 15`
...
专门的 K 线图页面，把价格聚合成 OHLC（开盘/最高/最低/收盘）：
...
价格记录是你的私有数据，编辑删除立即生效，不需审核。

> 你删除的只是你这条记录；别人记的同商品价格不受影响。

## 审核

价格记录本身是私有的，不审核。但如果你想改的是**商品或原料**（比如商品名、原料别名、商品权重等共享数据），那部分走提议-审核。详见 [原料与商品](ingredients.md) 和 [提议审核台](admin/review.md)。
